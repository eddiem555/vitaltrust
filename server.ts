...
import { MCP_TOOLS_SPEC } from "./mcp-tool-definitions";
...
async function startServer() {
  ...
  if (APP_ROLE === "aibroker" || IS_STANDALONE) {
    const executeMcpToolShared = createMcpExecutor({
      port: PORT,
      ...
      dbMock: db_mock,
      createLog: (userId, userName, role, action, status, details) =>
        createLog(userId, userName, role, action, status, details),
    });

    const mcpEnabled = process.env.MCP_ENABLED !== "false";
    mountMcpHttpServer(app, {
      version: VERSION,
      executeTool: executeMcpToolShared,
      getUsers: () => db_mock.users || [],
      enabled: mcpEnabled,
    });
    ...
    app.post("/api/ai/test-defense", async (req, res) => {
      ...
      try {
        ...
        const testUrl = buildInspectChatUrl(gatewayUrl);
    ...
    app.post("/api/ai/chat", async (req, res) => {
      ...
      // MCP tool schemas: mcp-tool-definitions.ts | execution: executeMcpToolShared
      const executeMCPTool = executeMcpToolShared;
